...
Exposure-bracket protection is now part of the culling/grouping foundation: Analyze evidence can identify conservative 3/5/7/9-frame same-composition exposure ladders, Cull preserves those members instead of treating them as ordinary duplicates, full bracket Moments are protected from semantic splitting, and Reference batch setup prefers an eligible center exposure. Actual HDR RAW merging remains intentionally outside the implemented path until the RAW renderer/merge/color/metadata boundary is production-grade; Lightroom/Camera Raw remains the merge destination for now.
